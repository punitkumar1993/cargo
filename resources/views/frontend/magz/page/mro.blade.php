                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="active">{{ ($term->name) ?? 'MROs' }}</li>
                            </ol>
                            @if(isset($term->name) && $term->name!=null)
                                <h1 class="page-title">{{ $term->name }}</h1>
                                <p class="page-subtitle">{{ __('Showing all mro with category') }}  <i>{{ $term->name }}</i></p>
                            @else
                                <h1 class="page-title">{{ __('MROs') }}</h1>
                                <p class="page-subtitle">{{ __('Showing all MROs') }} </p>
                            @endif
                        </div>
                    </div>
                    <div class="line"></div>
                    <div class="row">
                        @foreach ( $paginate_posts as $post)
                            <article class="col-md-12 article-list">
                                <div class="inner">
                                    <figure>
                                        <a href="{{ Settings::getRouteMro($post) }}">
                                    ...
                                    <div class="details">
                                        ...
                                        <h1><a href="{{ Settings::getRouteMro($post) }}">{{ $post->post_title }}</a></h1>
                                        ...
                                        <footer>
                                            ...
                                            <a class="btn btn-primary more" href="{{ Settings::getRouteMro($post) }}">
                                                <div>More</div>
